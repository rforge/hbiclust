ProcessRowDendrogram <- function(n_merge, 
								 merge_matrix, 
								 height_vector, 
								 order, 
								 cut_hight)
...
ProcessColDendrogram <- function(n_merge, 
								 merge_matrix, 
								 height_vector, 
								 order, 
								 cut_hight)
...
ProcessClusterOnRect <- function(size, 
								 size_ratio, 
								 margin, 
								 row_limits, 
								 col_limits,
								 n_color = 0)
{
	...
	out <- .C("ProcessClusterOnRect", 
			  ...
			  n_color = as.integer(n_color),
			  PACKAGE='hbiclust');

	return(list(rect_points=out$rect_points, n_clust = n_clust));
}
